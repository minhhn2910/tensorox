...
//#define TRAIN_DATA_NO_POSTPROCESS
#ifdef TRAIN_DATA_NO_POSTPROCESS
    printf("writing output back to file for training \n");

    {
      FILE *fp;
      fp = fopen("output_noprocess.txt", "w");
      for (int i = 0; i < OPT_N; i++)
      {

            fprintf(fp, ",%f",callValueBS[i]);
      }
      fclose(fp);
    }
#endif
//#define TEST_DATA_NO_PREPROCESS
#ifdef TEST_DATA_NO_POSTPROCESS
    printf("writing input back to file for training \n");

    {
      FILE *fp;
      fp = fopen("test_input_noprocess.txt", "w");
      for (int i = 0; i < OPT_N; i++)
      {
    ...
    {
      FILE *fp;
      fp = fopen("test_output_noprocess.txt", "w");
      for (int i = 0; i < OPT_N; i++)
      {
