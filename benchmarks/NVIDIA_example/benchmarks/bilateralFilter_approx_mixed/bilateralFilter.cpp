...
#define MAX_EPSILON_ERROR   0.1f//5.0f
...
int runSingleTest(char *ref_file, char *exec_path)
{
    ...
    // run the sample radius
    {
        ...
        sdkSavePPM4ub((const char *)dump_file, (unsigned char *)hResult, width, height);
          ...
          float* err_array = (float*)malloc(width* height*4*sizeof(float)) ;

                unsigned char  *ref_data;
                sdkLoadPPM4ub(sdkFindFilePath(ref_file, exec_path), &ref_data, &width, &height);
                unsigned char  *new_data = (unsigned char *)hResult;
                double rel_err = 0;
                double rel_err_2 = 0;
                double abs_err = 0;
                for (int i =0; i< width* height*4 ; i++){
                  ...
                  {
                    if(ref_data[i]!=0){
                      rel_err += fabs((new_data[i]-float(ref_data[i]))/float(ref_data[i]));
                      err_array[i] = fabs((new_data[i]-float(ref_data[i]))/float(ref_data[i]));
                    }
                    abs_err +=fabs(new_data[i]-float(ref_data[i]));
                    rel_err_2 += fabs((new_data[i]-float(ref_data[i]))/256);
                  }
                }
                /*
                {
                  ...
                  fprintf(fp, "%f", err_array[0]);
                  for (int i=1; i <width* height*4; i++ ){
                    fprintf(fp, ",%f", err_array[i]);
                  }
                  fclose(fp);
                }
                */

          printf(" avg rel err %f , avg abs_err %f rel_err_2 %f\n",rel_err/(width* height*4 ), abs_err/(width* height*4 ), rel_err_2/(width* height*4 ) );


        if (!sdkComparePPM(dump_file, sdkFindFilePath(ref_file, exec_path), MAX_EPSILON_ERROR, 0.0015f, false))
        {
