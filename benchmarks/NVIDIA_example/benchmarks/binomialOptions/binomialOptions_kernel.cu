...
//#define TRAINING_DATA_NO_PREPROCESS
#ifdef TRAINING_DATA_NO_PREPROCESS
    printf("writing input back to file for training \n");

    {
      FILE *fp;
      fp = fopen("input_noprocess.txt", "a");
      for (int i = 0; i < optN; i++)
      {
            fprintf(fp, ",%f,%f,%f,%f,%f",optionData[i].S,optionData[i].X,optionData[i].T,
                optionData[i].R, optionData[i].V);
      }
      fclose(fp);
    }
    ...
    {
      FILE *fp;
      fp = fopen("output_noprocess.txt", "a");
      for (int i = 0; i < optN; i++)
      {

            fprintf(fp, ",%f",callValue[i]);
      }
      fclose(fp);
    }
#endif
...

    {
      ...
      for (int i = 0; i < optN; i++)
      {
            fprintf(fp, ",%f,%f,%f,%f,%f",h_OptionData[i].S,h_OptionData[i].X,h_OptionData[i].vDt,
                h_OptionData[i].puByDf, h_OptionData[i].pdByDf);
      }
      fclose(fp);
    }
...
#endif
//#define TEST_DATA_NO_PREPROCESS
#ifdef TEST_DATA_NO_PREPROCESS
printf("writing input back to file for training \n");

{
  FILE *fp;
  fp = fopen("test_input_noprocess.txt", "a");
  for (int i = 0; i < optN; i++)
  {
        fprintf(fp, ",%f,%f,%f,%f,%f",optionData[i].S,optionData[i].X,optionData[i].T,
                optionData[i].R, optionData[i].V);
...
{
  FILE *fp;
  fp = fopen("test_output_noprocess.txt", "a");
  for (int i = 0; i < optN; i++)
  {
