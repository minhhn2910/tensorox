...
__global__ void fun_gpu(float x[], float y[], int nthreads, float h){
  ...
  int base_address = blockIdx.x*256*4;
  ...
  //  __shared__ half A[8][512];
	...
	//	if (tid < nthreads-1){
			  neuron_out[warp_id][warp_lane+ 0*32] = x[4*tid];
			  neuron_out[warp_id][warp_lane+ 1*32] = x[4*tid+1];
			  neuron_out[warp_id][warp_lane+ 2*32] = x[4*tid+2];
			  neuron_out[warp_id][warp_lane+ 3*32] = x[4*tid+3];
		// }
...
__shared__ float temp_out[256*4];

temp_out[real_tid*4] = __half2float(output1.x)*1e-6;
temp_out[real_tid*4+1] = __half2float(output1.x)*1e-6;
temp_out[real_tid*4+2] = __half2float(output1.y)*1e-6;
temp_out[real_tid*4+3] = __half2float(output1.y)*1e-6;

//  if (tid*4 < nthreads-1) {
    y[base_address+real_tid] =  temp_out[real_tid];
    y[base_address+real_tid+256] =  temp_out[real_tid+256];
    y[base_address+real_tid+256*2] =  temp_out[real_tid+256*2];
    y[base_address+real_tid+256*3] =  temp_out[real_tid+256*3];
//  }
}
...
int main( int argc, char **argv) {
  ...
  int block_count = ((n + threads_per_block - 1)/threads_per_block)/4;
