...
__global__ void nrpol3_kernel(float *A_coeff, float *B_coeff, float *C_coeff, float *D_coeff, float *x0_in,
  ...
  , float *root, int size, float err_thresh, int speed)
{
...
//        return;
    ...
    if (tid*2 < size) {
      ...
      int tid = blockDim.x * blockIdx.x + threadIdx.x;
  		...
  		float x0 = x0_in[2*tid];
  		float x1, fx, fdx;
  		float temp_result = 0;//store temp_res in loop
  		for (int i = 0; i < MAX_LOOP; i++) {
  			//fx  = F(x0);
  			fx = (A_coeff[2*tid]*x0*x0*x0)+(B_coeff[2*tid]*x0*x0)+(C_coeff[2*tid]*x0)+D_coeff[2*tid];
  			//fdx = FD(x0);
  			fdx = (3.0*A_coeff[2*tid]*x0*x0)+(2.0*B_coeff[2*tid]*x0)+(C_coeff[2*tid]);
  			x1  = x0 - (fx/fdx);
  		...
  		root[2*tid] = temp_result;

      curr_err = MAX_DIFF * err_thresh;
       x0 = x0_in[2*tid+1];
  		 temp_result = 0;//store temp_res in loop
  		for (int i = 0; i < MAX_LOOP; i++) {
  			//fx  = F(x0);
  			fx = (A_coeff[2*tid+1]*x0*x0*x0)+(B_coeff[2*tid+1]*x0*x0)+(C_coeff[2*tid+1]*x0)+D_coeff[2*tid+1];
  			//fdx = FD(x0);
  			fdx = (3.0*A_coeff[2*tid+1]*x0*x0)+(2.0*B_coeff[2*tid+1]*x0)+(C_coeff[2*tid+1]);
  			x1  = x0 - (fx/fdx);
  			if (fabs((x1-x0) / x1) < curr_err) {
  				curr_err = fabs((x1-x0) / x1);
  				temp_result = x1;
...
#pragma parrot.start("nrpol3_kernel")
    ...
    , root_d, data_size, err_thresh,speed);
