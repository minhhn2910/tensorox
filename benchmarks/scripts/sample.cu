...
//tensorox_input:xTarget_in[2*tid]:yTarget_in[2*tid]:xTarget_in[2*tid+1]:yTarget_in[2*tid+1]
	if(idx < size)
	{
//		float parrotInput[2];
...
//tensorox_output:angles[(2*tid)*3+0]:angles[(2*tid)*3+1]:angles[(2*tid)*3+2]:angles[(2*tid+1)*3+0]:angles[(2*tid+1)*3+1]:angles[(2*tid+1)*3+2]
}
